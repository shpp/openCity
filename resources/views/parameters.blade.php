              ...
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    ...
                    <select class="selectpicker" name="parameter_type_id" form="add" id="sel" data-width="100%">
                    @foreach ($parameterTypes as $type)
                        <option value="{{ $type->id }}" data-thumbnail="{{ $type->icon }}" style="background: #bfffd6;">{{ $type->name }}</option>
                    @endforeach
                    </select>
                    {{ csrf_field() }}
                    </form>
                        ...
                        <tbody>
                            @foreach ($parameters as $param)
                                <tr>
                                    ...
                                    <td class="table-text">
                                        <div>
                                            <input type="text" class="form-control" name="comment" form="form{{$param->id}}" value="{{ $param->comment }}">
                                        </div>
                                    </td>
                                    <td class="table-text">
                                        <div>
                                          <select class="selectpicker" name="parameter_type_id" form="form{{$param->id}}" id="sel{{$param->id}}" data-width="100%">
                                            @foreach ($parameterTypes as $type)
                                                @if ($type->id === $param->parameter_type_id)
                                                <option selected value="{{ $type->id }}" data-thumbnail="{{ $type->icon }}" style="background: #bfffd6;">{{ $type->name }}</option>
                                                @else
                                                <option value="{{ $type->id }}" data-thumbnail="{{ $type->icon }}" style="background: #bfffd6;">{{ $type->name }}</option>
                                                @endif
                                            @endforeach
